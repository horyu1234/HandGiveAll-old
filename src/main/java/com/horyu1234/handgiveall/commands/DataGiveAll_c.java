			...
			if (l.equalsIgnoreCase("dga")) {
				...
				if (a[0].equalsIgnoreCase("?")) {
					...
					if (contain(a[1])) {
						...
						PlayerUtils.sendMsg(plugin.bcprefix+String.format("§e%s§a님이 모든 온라인 플레이어에게 §e%s§a을(를) §e%s개 §a지급하였습니다.", plugin.config_use_nickname ? (s instanceof Player) ? ((Player) s).getDisplayName() : s.getName() : s.getName(), plugin.getItemUtils().getItemName(item), fmt.format(amount)));
					...
					if (contain(a[1]))
					{
						...
						PlayerUtils.sendMsg("");
						PlayerUtils.sendMsg(plugin.bcprefix+"§3=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=");
						PlayerUtils.sendMsg(plugin.bcprefix+String.format("§e%s§a님이 모든 온라인 플레이어에게 §e%s§a을(를) §e(랜덤추첨된)%s개 §a지급하였습니다.", plugin.config_use_nickname ? (s instanceof Player) ? ((Player) s).getDisplayName() : s.getName() : s.getName(), plugin.getItemUtils().getItemName(item), fmt.format(random)));
